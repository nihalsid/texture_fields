...
def read_points_occ(path):
    ...
    print(points.shape, occupancies.shape)
    return points[occupancies == 1, :]


def depthmap_to_world(depth, cam_W, cam_K):
    ...
    loc3d = loc3d.reshape(loc3d.shape[0]*loc3d.shape[1], loc3d.shape[2]).numpy()/ 96 - 0.5
...
if __name__ == '__main__':
    # points_to_obj(read_points_occ('test_data/points.npz'), "test_data/out.obj")
    depth, cam_W, cam_K = read_depth_and_cameras_as_tensors("test_data_01/sdf.npz", "test_data_01/render", 0)
    points_to_obj(depthmap_to_world(depth, cam_W, cam_K), "test_data_01/depth.obj")
